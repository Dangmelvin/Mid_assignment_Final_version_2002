...
# Function to measure the performance using timeit
def measure_performance():
    setup = '''
from __main__ import floyd, graph
    '''
    stmt = '''
floyd(graph)
    '''
    # Run the code using timeit
    execution_time = timeit.timeit(stmt=stmt, setup=setup, number=1)
    print("Execution time for Floyd-Warshall algorithm: {:.6f} seconds".format(execution_time))
